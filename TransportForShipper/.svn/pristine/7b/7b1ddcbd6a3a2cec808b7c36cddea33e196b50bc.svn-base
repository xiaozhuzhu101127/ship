...
public class Dict {
	...
	public static boolean isMunicipalCity(String cityCode){
		...
		if(cityCode.equals("BJ")||cityCode.equals("SH")||cityCode.equals("TJ")||cityCode.equals("ZQ")){
			return true;
		}
		return false;
	}
	...
	public static String getLocation(String cityCode){
		...
		if(cityCode.indexOf("_") != -1 && !Dict.isMunicipalCity(cityCode.split("_")[0]))
	...
	public static String getCityName(String cityCode, int level){
		...
		//目前只支持两级
		if(codes.length == 3 && !Dict.isMunicipalCity(codes[0])){
			cityCode = cityCode.substring(0, cityCode.lastIndexOf("_"));
			codes = cityCode.split("_");
		}
		if(level == 1){
	...
	public static Map<String, String> getGsLoadDict(){
		if(GSLOAD == null){
			...
			GSLOAD.put("GSLOAD_NOW","一装一卸");
			GSLOAD.put("GSLOAD_NextDay","多装多卸");
			GSLOAD.put("GSLOAD_WithTK","一装两卸");
		}
		return GSLOAD;
	}
