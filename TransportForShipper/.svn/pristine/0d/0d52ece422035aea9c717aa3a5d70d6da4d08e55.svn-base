...
implements Runnable {
	...
	private void downloadApk() {
		...
		} else if (getCacheDirAvailableSpaces()> NEED_SPACE){
			int p = mDownloadUrl.lastIndexOf("/");
			String fileName = mDownloadUrl.substring(p);
			apkFile = new File(MyApplication.getInstance().getCacheDir().getPath()+fileName);
		}else {
		...
		try {
			...
			e.printStackTrace();
			Log.e("apkDownloadTask", e.getMessage());
			if (mApkDownloadListener != null) {
				mApkDownloadListener.downloadFail();
			}
		} finally {
	...
	private long getCacheDirAvailableSpaces(){
	    File path = MyApplication.getInstance().getCacheDir();
